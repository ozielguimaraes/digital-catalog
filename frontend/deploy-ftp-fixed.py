#!/usr/bin/env python3
"""
Script corrigido para deploy via FTP
Lida com problemas de timeout e navegação
"""
...
def upload_via_ftp(zip_name):
    """Faz upload via FTP com configurações otimizadas"""
    print_message("🌐 Conectando via FTP...", Colors.YELLOW)
    
    try:
        # Conectar ao FTP com configurações otimizadas
        ftp = ftplib.FTP()
        ftp.set_debuglevel(0)  # Desativar debug
        ftp.set_pasv(True)     # Usar modo passivo
        ...
        try:
            current_dir = ftp.pwd()
            print_message(f"📁 Diretório atual: {current_dir}", Colors.BLUE)
        except:
            print_message("⚠️  Não foi possível obter diretório atual", Colors.YELLOW)
        
        # Tentar navegar para public_html
        try:
            ftp.cwd(FTP_CONFIG['remote_dir'])
            print_message(f"✅ Navegou para {FTP_CONFIG['remote_dir']}", Colors.GREEN)
        except:
            print_message(f"⚠️  Não foi possível navegar para {FTP_CONFIG['remote_dir']}", Colors.YELLOW)
            print_message("📋 Tentando listar diretórios disponíveis...", Colors.YELLOW)
            try:
                ftp.retrlines('LIST')
            except:
                print_message("⚠️  Não foi possível listar diretórios", Colors.YELLOW)
        ...
        print_message("✅ Upload concluído!", Colors.GREEN)
...
def main():
    print_header("🚀 DEPLOY AUTOMÁTICO VIA FTP (CORRIGIDO)")
    ...
    if not create_zip(zip_name, "dist/ng-tailadmin"):
        sys.exit(1)
    
    # Upload
    print_header("🌐 UPLOAD VIA FTP")
    
    if not upload_via_ftp(zip_name):
        print_message("❌ Falha no upload FTP", Colors.RED)
        print_message("💡 Tente fazer upload manual do arquivo ZIP", Colors.YELLOW)
        print_message(f"📁 Arquivo: {zip_name}", Colors.BLUE)
        sys.exit(1)
    ...
    print_message("3. Navegue até o diretório correto (public_html, www, etc.)", Colors.YELLOW)
