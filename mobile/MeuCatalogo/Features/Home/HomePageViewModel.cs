...
public sealed partial class HomePageViewModel : BasePageViewModel
{
    ...
    [ObservableProperty] private string? _catalogoFavoritoNome;
    ...
    [ObservableProperty] private bool _temCatalogoFavorito;
    ...
    private async Task Carregar()
    {
        try
        {
            ...
            var favorito = _settingsService.CatalogoFavorito;
            TemCatalogoFavorito = favorito is not null;
            CatalogoFavoritoNome = favorito?.Nome;

            if (favorito is not null)
            {
                var produtos = (await _produtoLocalRepository.GetByCatalogoIdAsync(favorito.Id.ToString())).ToList();
