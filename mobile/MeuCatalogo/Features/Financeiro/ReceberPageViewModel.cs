...
public sealed partial class ReceberPageViewModel(
    ILogger<ReceberPageViewModel> logger,
    ...
    INavigationService navigationService) : BasePageViewModel
{
    ...
    private async Task Carregar()
    {
        try
        {
            ...
            var response = await getLancamentosUseCase.ExecuteAsync(LancamentoTipo.Receber);
            if (response.RetornouComErro)
            {
                logger.LogWarning("Erro ao carregar a receber: {Mensagem}", response.MensagemDeErro);
        ...
        {
            logger.LogError(ex, "Erro ao carregar a receber");
        }
        finally
        {
            IsLoading = false;
